...
bool Level::init()
{
	...
	Config* config = ((AppDelegate*)Application::getInstance())->getConfigClass();
	Json::Value currentLevelConfig = config->getConfigValues(Config::ConfigType::LEVEL)["worlds"][id_world]["levels"][id];
	...
	Json::Value towerConfigurations = config->getConfigValues(Config::ConfigType::TOWER);
	std::vector<Tower::TowerType> allTypes = TowerFactory::getAllTowerTypes();
	for (unsigned int i(0); i < allTypes.size(); ++i) {
		towersSettings[allTypes[i]] = TowerSettings::create(towerConfigurations, TowerFactory::getTowerNameFromType(allTypes[i]));
	}
...
TowerSettings * Level::getTowerSettings(Tower::TowerType type)
{
