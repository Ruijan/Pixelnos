#include "Bomber.h"
#include "Attacks/ThrowableAttacks/WaterBall.h"
#include "Attacks/ThrowableAttacks/WaterBombBall.h"
#include "../Level.h"
...
void Bomber::attack(){
	if (target != nullptr) {
		...
		if (level >= settings->getMaxExistingLevel() - 1) {
			ball = WaterBombBall::create(globalLevel, target, settings->getDamage(level), 500 * visibleSize.width / 960, 100);
		}
		else {
