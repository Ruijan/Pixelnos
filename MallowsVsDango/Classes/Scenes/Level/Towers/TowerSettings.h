...
public:
	...
	static TowerSettings * create(const Json::Value & configuration, const std::string & towerName);
	virtual void init(const std::string& configFilename, const std::string & towerName);

	void init(const Json::Value & configuration, const std::string & towerName);

	void fillDatabase(const std::string & towerName);
